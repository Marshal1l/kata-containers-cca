...
func Socket(domain, typ, proto int, name string, cfg *Config) (*Conn, error) {
	...
	sb.WriteString("socket_err content:\n")

	// 关键：打印 Domain (协议族), Type (类型), Proto (协议)
	// Domain 10 = AF_INET6 (IPv6), Domain 16 = AF_NETLINK, Domain 40 = AF_VSOCK
	sb.WriteString(fmt.Sprintf("Domain: %d\n", domain))
	sb.WriteString(fmt.Sprintf("Type:   %d\n", typ))
	sb.WriteString(fmt.Sprintf("Proto:  %d\n", proto))
	sb.WriteString(fmt.Sprintf("Name:   %s\n", name))
	sb.WriteString("--------------------------\n")
	os.WriteFile("/home/user/mzh_conn.log", []byte(sb.String()), 0644)
	if cfg.NetNS == 0 {
