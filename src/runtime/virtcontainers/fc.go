...
func (fc *firecracker) fcInit(ctx context.Context, timeout int) error {
	...
	if fc.jailed {
		...
		args = append(args,
			"--no-seccomp", "--api-sock", fc.socketPath,
...
func (fc *firecracker) fcSetVMBaseConfig(ctx context.Context, mem int64, vcpus int64, smtEnabled bool, ccaEnable bool) {
	...
	if ccaEnable {
		...
		cfg := &models.MachineConfiguration{
			...
			MemSizeMib: &mem,
			VcpuCount:  &vcpus,
			RealmConfig: &models.RealmConfig{
...
func (fc *firecracker) fcInitConfiguration(ctx context.Context) error {
	...
	fc.fcSetVMBaseConfig(ctx, int64(fc.config.MemorySize),
		int64(fc.config.NumVCPUs()), false, fc.config.ConfidentialGuest)
...
// actually StartVM already exec cmd to start the virtual machine
